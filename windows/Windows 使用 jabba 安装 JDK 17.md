## Windows 上使用 jabba 安装并管理 JDK 17（含问题排查）

本文档适用于 Windows 10/11 的 PowerShell 环境，指导通过 jabba 安装并设置 JDK 17 为默认版本，并给出常见问题的处理方法。
...
### 4. 安装 JDK 17（推荐 Zulu 或 Temurin 发行版）
列出可用 17 版本（可选）：
```powershell
...
安装（任选其一）：
```powershell
# Zulu（示例最终解析为 zulu@17.0.9）
jabba install zulu@17

# 或 Temurin（推荐，Eclipse Adoptium 的继任者）
...
jabba alias default zulu@17

# 方式 B：在 profile 中显式使用解析后的具体版本（稳定可靠）
. "$env:JABBA_HOME\jabba.ps1"
$jdk = (jabba which zulu@17)  # 例如 C:\Users\<user>\.jabba\jdk\zulu@17.0.9
$ver = Split-Path $jdk -Leaf    # 例如 zulu@17.0.9
...
OpenJDK Runtime Environment Zulu17.50.19-CA (build 17.0.9+9-LTS)
OpenJDK 64-Bit Server VM Zulu17.50.19-CA (build 17.0.9+9-LTS, mixed mode, sharing)
```
...
1) 安装/解析 JDK 17 并在当前会话生效
```powershell
& "$HOME\.jabba\bin\jabba.exe" install zulu@17
$jdk = & "$HOME\.jabba\bin\jabba.exe" which zulu@17
$env:JAVA_HOME = $jdk
...
$lines += '$jdk = & "$env:JABBA_HOME\bin\jabba.exe" which zulu@17'
$lines += 'if ($jdk) {'
...
### 10. JDK 17 新特性说明
- **模块系统**：JDK 17 支持 Java 模块系统（JPMS），可以更好地管理依赖。
- **文本块**：支持多行字符串字面量，提高代码可读性。
- **Switch 表达式**：更简洁的 switch 语句语法。
- **Records**：用于创建不可变数据类的简洁语法。
- **密封类**：限制类的继承层次结构。

### 11. 推荐发行版对比
- **Zulu**：Azul 提供，企业级支持，性能优化
- **Temurin**：Eclipse Adoptium 提供，社区驱动，免费
- **Oracle OpenJDK**：Oracle 官方构建，免费但无长期支持

建议选择 **Temurin** 或 **Zulu**，两者都有良好的社区支持和长期维护。
